...
void Modbus::showHregs()
{
    TRegister *reg = _regs_head;
    if (reg == 0)
    {
        Serial.println("Modbus: No Hreg registred");
        return;
    }
    else
    {
        Serial.println("\nModbus: Hreg registred:");
        do
        {
            Serial.print(F("Addresss:"));
            Serial.print(reg->address - 40001);
            Serial.print(F("\tvalue:"));
            Serial.print(reg->value);
            Serial.print(F("\tpreviousValue:"));
            Serial.print(reg->previousValue);
            Serial.println();
            reg = reg->next;
        } while (reg);
    }
...
word Modbus::checkHregUpdate()
{
    ...
    if (reg != 0)
    {
        do
        {
            if (reg->value != reg->previousValue)
            {
                reg->previousValue = reg->value;
                return reg->address-40001;
            }
            reg = reg->next;
        } while (reg);
    }
